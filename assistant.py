...
def generate_response():
    ...
    user_id = data.get('user_id')  # Identificar al usuario por su ID único

    if not user_message or not user_id:
        return jsonify({'response': "No se proporcionó un mensaje o un ID de usuario válido."}), 400

    try:
        # Obtener el hilo existente para el usuario o crear uno nuevo si no existe
        thread_id = user_threads.get(user_id)
        if not thread_id:
            # Crear un nuevo hilo de conversación si no existe para el usuario
            ...
            thread_id = thread.id
            user_threads[user_id] = thread_id  # Asociar el hilo al usuario
